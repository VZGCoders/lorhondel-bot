...
class PetRepository extends AbstractRepository
{
    ...
    public function new($author_id = null, $species = null, $name = null, $class = null): string
    {
		...
		if ($part = $this->factory->lorhondel->factory(\Lorhondel\Parts\Pet\Pet::class, [
			...
			if (! $species || ! in_array(trim($species), $part::getFillableSpeciesAttributes())) {
				$return = 'Please tell us the species you want for your Pet in the following format: `' . $this->factory->lorhondel->command_symbol . 'pet create {species}` where `{species}` is any of the following:' . PHP_EOL;
			...
			$this->save($part);
			return 'Created pet `' . $part->name . ' ` with ID `' . $part->id . '`. You can make this your active Pet by using the command `' . $this->factory->lorhondel->command_symbol . 'pet activate ' . $part->id . '`.';				
    ...
    public function activate($author_id = null, $id = null): string
    {
		if (! ($id instanceof Pet)) {
			if (! is_numeric($id)) return "You must include the numeric ID of the Pet you want to activate! You can check `{$this->factory->lorhondel->command_symbol}pets` if you need a list of your IDs!";
